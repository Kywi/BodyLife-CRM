...
public sealed class PostgreSqlListLowRemainingMembershipsQueryTests
{
    ...
    public async Task QueryUsesCanonicalStateWithStableFilteringOrderingAndPagination()
    {
        ...
        var negative = await InsertMembershipAsync(
            database,
            fixture,
            "Negative",
            remainingVisits: -1);
        var zero = await InsertMembershipAsync(
            database,
            fixture,
            "Zero",
            remainingVisits: 0);
        ...
        var zulu = await InsertMembershipAsync(
            ...
            "Zulu",
            remainingVisits: 1);
        var two = await InsertMembershipAsync(
            database,
            fixture,
            "Two",
            remainingVisits: 2);
        ...
        var fullResult = await reportHandler.ExecuteAsync(
            new ListLowRemainingMembershipsQuery(
                fixture.Actor,
                AsOfDate,
                Limit: GetLowRemainingMembershipStateRowsQuery.MaxLimit),
        ...
        Assert.Equal([-1, 0, 1, 1, 2], fullPage.Items.Select(row => row.RemainingVisits));
        Assert.Equal(
        ...
        Assert.Equal(
            ...
            fullPage.Items[1].Warnings.Select(warning => warning.Code));
        Assert.Equal(
            [MembershipWarningCodes.LowRemaining],
            fullPage.Items[2].Warnings.Select(warning => warning.Code));
        Assert.All(fullPage.Items, row => Assert.Equal(AsOfDate, row.MembershipState.AsOfDate));
        Assert.False(fullPage.HasMore);
        Assert.Null(fullPage.NextOffset);

        var zeroThresholdResult = await reportHandler.ExecuteAsync(
            new ListLowRemainingMembershipsQuery(
                fixture.Actor,
                AsOfDate,
                RemainingVisitsThreshold: 0),
            CancellationToken.None);
        AssertSuccessful(zeroThresholdResult);
        Assert.Equal(
            [negative.MembershipId, zero.MembershipId],
            zeroThresholdResult.Page!.Items.Select(row => row.MembershipId));
        ...
        Assert.Equal(
            [alpha.MembershipId, zulu.MembershipId],
            secondPageResult.Page!.Items.Select(row => row.MembershipId));
        Assert.True(secondPageResult.Page.HasMore);
        Assert.Equal(4, secondPageResult.Page.NextOffset);

        var alphaRow = fullPage.Items[2];
        ...
        Assert.Equal(canonicalState.State.CountedVisits, alphaRow.CountedVisits);
        Assert.Equal(
            canonicalState.State.Snapshot.VisitsLimit,
            alphaRow.VisitsLimitSnapshot);
        ...
        Assert.Contains(
            ...
            await ReadRemainingVisitsQueryPlanAsync(database),
    ...
    public async Task AuthorizationPrecedesValidationAndInvalidSelectorsCarryNoPage()
    {
        ...
        AssertFailure(denied, ListLowRemainingMembershipsStatus.PermissionDenied);
        AssertFailure(
            invalidThreshold,
    ...
    public async Task SourceFailuresNeverReturnPartialReportRows()
    {
        ...
        {
            (
                Source: GetLowRemainingMembershipStateRowsResult.Denied(),
                Expected: ListLowRemainingMembershipsStatus.PermissionDenied,
                Field: (string?)null),
            (
                Source: GetLowRemainingMembershipStateRowsResult.Invalid(
                    "Limit is invalid.",
                    "limit"),
                Expected: ListLowRemainingMembershipsStatus.ValidationFailed,
                Field: "limit"),
            (
                Source: GetLowRemainingMembershipStateRowsResult.RecalculationFailed(),
                Expected: ListLowRemainingMembershipsStatus.RecalculationFailed,
                Field: (string?)null),
            (
                Source: GetLowRemainingMembershipStateRowsResult.InconsistentSource(),
                Expected: ListLowRemainingMembershipsStatus.SourceInconsistent,
                Field: (string?)null),
        };

        foreach (var scenario in scenarios)
        {
            var result = await new ListLowRemainingMembershipsQueryHandler(
                    new StubSourceHandler(scenario.Source))
                .ExecuteAsync(
                    new ListLowRemainingMembershipsQuery(actor, AsOfDate),
                    CancellationToken.None);
    ...
    public void PersistenceRegistrationExposesScopedSourceAndReportHandlers()
    {
        ...
        AssertScopedRegistration<
            ...
            GetLowRemainingMembershipStateRowsQueryHandler>(services);
        AssertScopedRegistration<
            ListLowRemainingMembershipsQuery,
            ListLowRemainingMembershipsResult,
            ListLowRemainingMembershipsQueryHandler>(services);
    ...
    private static ListLowRemainingMembershipsQueryHandler CreateHandler(
        BodyLifeDbContext dbContext)
    {
        return new ListLowRemainingMembershipsQueryHandler(
            new GetLowRemainingMembershipStateRowsQueryHandler(
                dbContext,
                new FixedTimeProvider(TestNow)));
    }

    private static async Task<LowRemainingFixture> SeedFixtureAsync(
        ...
        BodyLifeDbContext dbContext)
    {
        ...
        return new LowRemainingFixture(
            new ActorContext(
        ...
        bool includeCache = true)
    {
        ...
        command.CommandText =
            ...
            values (
                gen_random_uuid(), @membership_id, @start_date, @remaining_visits,
                @negative_balance, @effective_end_date, @extension_days,
                'Low remaining report fixture',
                'Historical state required by the report scenario', @recorded_at,
            ...
            select
                ...
                @remaining_visits,
                @negative_balance,
                case when @remaining_visits < 0 then @first_negative_visit_id else null end,
                case when @remaining_visits < 0 then @first_negative_visit_date else null end,
                @extension_days,
                @effective_end_date,
                @last_counted_visit_at,
                @recalculated_at,
    ...
    private static async Task<string> ReadRemainingVisitsQueryPlanAsync(
        PostgreSqlTestDatabase database)
    {
        await using var connection = new NpgsqlConnection(database.ConnectionString);
        await connection.OpenAsync();
        ...
        command.CommandText =
            ...
            where remaining_visits <= @remaining_visits_threshold
            order by remaining_visits
            """;
        command.Parameters.AddWithValue(
            "remaining_visits_threshold",
    ...
    private static void AssertSuccessful(ListLowRemainingMembershipsResult result)
    {
        Assert.Equal(ListLowRemainingMembershipsStatus.Success, result.Status);
    ...
    private static void AssertFailure(
        ListLowRemainingMembershipsResult result,
        ListLowRemainingMembershipsStatus expectedStatus,
    ...
    private sealed record LowRemainingFixture(
        ActorContext Actor,
        Guid AccountId,
    ...
    private sealed class StubSourceHandler(
        ...
        : IBodyLifeQueryHandler<
            ...
            GetLowRemainingMembershipStateRowsResult>
    {
        public Task<GetLowRemainingMembershipStateRowsResult> ExecuteAsync(
