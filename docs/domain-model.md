...
Accepted ADR-012 уточнює повноваження v1: додавання і скасування NonWorkingDays є Owner-only, хоча ранні вимоги згадували Owner або Admin. ADR-016 визначає affected scope: lifecycle-active Membership із будь-яким inclusive overlap з locked pre-command canonical date interval отримує весь confirmed period. Домен має показати exact affected Membership set та applied range перед підтвердженням і зберегти reason кожного extension.
...
- NonWorkingDay eligibility uses lifecycle-active Membership and any inclusive
  overlap with canonical state calculated without the proposed/replaced period.
  Every confirmed application contributes the full inclusive period; it is not
  clipped to Membership start or pre-command effective end.
- Confirmed NonWorkingDay application scope is an immutable transaction
  snapshot. Later Membership/source changes do not silently add or remove
  applications; retroactive scope change requires an explicit audited workflow.
...
2. System previews the exact lifecycle-active Membership set whose canonical
   interval has any inclusive overlap, plus the full applied range for each.
3. Owner confirms the expiring scope snapshot; command revalidates the same set
   and ranges in its transaction or fails `affected_scope_changed`.
4. Memberships recalculates affected effective end dates using the full period
   per application and union calendar-day rule.
...
Under ADR-016, `applicable` is not the clipped date intersection. Any inclusive
overlap with a lifecycle-active Membership's canonical pre-command interval
makes it eligible, and its confirmed application contributes the full period:

```text
eligible = period_start <= pre_command_effective_end
           and period_end >= membership_start
applied_range = period_start to period_end
```

The proposed/replaced period is excluded from the eligibility calculation so it
cannot make itself applicable. Current server date does not replace the date
overlap rule.
...
| NonWorkingDay | Owner adds non-working period | Any inclusive overlap makes a lifecycle-active Membership eligible; its application contributes the full period | Preview exact Membership set/full ranges before confirmation; Audit records owner and reason |
| NonWorkingDay | Membership starts or ends inside a non-working period | The confirmed application still contributes the complete period under ADR-016 | Preview makes the boundary outcome visible; union calculation prevents duplicate dates |
...
12. NonWorkingDay extends affected memberships
   - Given lifecycle-active Membership ends on 2026-01-01 and proposed NonWorkingDay period is 2026-01-01 to 2026-01-03.
   - When Owner confirms period.
   - Then all 3 period dates become extension source days, the exact scope is retained, and membership history shows reason.
