...
1. Confirm exact date arithmetic for `duration_days`: this document uses `start_date + duration_days - 1 day` to preserve inclusive end date. If the gym's current paper convention uses another formula, update domain tests before implementation.
2. Define whether NonWorkingDay period extends Membership only for overlapping active calendar days or for full period once any overlap exists.
3. Define validation for Freeze ranges: can Freeze start before Membership start, after Membership end, or outside current active period?
4. Decide whether recording Visit during active Freeze is blocked, warned, or allowed with explicit override.
5. Decide whether Client can have more than one active issued Membership at the same time, and how Visits choose Membership if overlap exists.
6. Finalize default workflow for Visit without active Membership: one-off/trial, warned negative visit, or explicit choice each time.
7. Specify exact one-off negative closure behavior: whether it consumes one-off MembershipType, payment-only closure, or another explicit domain fact.
8. Define which correction/cancellation actions always require reason/comment and which can use optional comments.
9. Define day close/reconciliation policy: who closes the day, what owner approval means after close, and how changed-after-close is labeled in Reports.
10. Choose standard inactive-client default threshold while keeping 14, 30 and 60 days available.
11. Decide whether one-off/trial visits use technical Client, dedicated quick-visit workflow, or both.
12. Define ordering for multiple Visits on same date if only date is recorded and first negative date must be derived. If date-level precision is enough, same-day ordering does not affect displayed date.
13. Define whether denied permission attempts are business-audited or only technically logged.
14. Decide how much historical card-assignment history is visible in v1 beyond current card number and Audit trail.
