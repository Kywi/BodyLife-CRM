...
Основа: `docs/adr/006-business-audit-corrections-and-technical-logs.md`, `docs/adr/009-backup-restore-and-operational-recovery.md`, `docs/adr/010-migration-manual-backfill-and-paper-fallback.md`, `docs/adr/017-business-time-zone-and-ui-localization.md` і `docs/interaction-contracts.md`.
...
- Зберігати canonical instants і technical-log timestamps у UTC, але трактувати
  business dates і owner/reception presentation через fixed `Europe/Kyiv`
  calendar та active UI culture.
...
- Stored audit instants remain UTC; display uses `Europe/Kyiv` and active culture
...
| Field | Policy |
|---|---|
| `timestamp` | Canonical UTC server-side time. Technical logs are not localized for UI. |
...
| Membership cache rebuild version/result | A stale derived cache can block profile/report reads after a contract change. | Before traffic, require current version, exit code 0 and retained totals; rerun after any interruption. |
...
### Membership cache version rebuild

The standalone Membership cache rebuild is an operational derived-state
release/repair path and is deliberately separate from the atomic
`AddNonWorkingDay`/`CorrectNonWorkingDay` workflows above.

When deployed code raises the Membership recalculation contract to version `7`:

1. Keep the application out of reception/owner traffic.
2. Apply ordinary reviewed EF migrations, if any.
3. Run `scripts/rebuild-membership-state-caches.sh` with the production
   connection/configuration and current application build.
4. Require exit code `0` and record technical-log totals for `total`, `created`,
   `repaired`, `verified` and recalculation version `7`.
5. Only then enable traffic and perform a canonical client-profile/report spot
   check.

The command enumerates issued Memberships deterministically and uses the same
canonical rebuilder as business commands. Each Membership commits separately,
so cancellation, process loss or a non-zero exit may leave valid partial
progress. This is expected operational behavior: source facts and business audit
are not changed, and recovery is to fix the reported cause and rerun the same
command until it exits `0`. Missing Membership/source failure logs the current ID
and processed counts. Do not mark release readiness complete from a partial run,
edit cache rows manually or convert this mechanism into a partial business
workflow.

Because only rebuildable derived state changes, the rebuild creates no business
audit entry. Its invocation, result, failure and counts belong to technical logs
and deployment evidence. After restore, run the same command before comparing
cache state or reopening traffic.
...
Business time and derived cache:

- [ ] Production configuration resolves the fixed `Europe/Kyiv` business zone;
  server/browser local zones do not alter command or query results.
- [ ] UTC storage/logging and localized `uk-UA`/`en-US` UI presentation have been
  checked without visible UTC/offset suffixes.
- [ ] DST gap/fold input and 23/24/25-hour daily query tests pass.
- [ ] Every issued Membership cache has recalculation version `7` before traffic.
- [ ] `scripts/rebuild-membership-state-caches.sh` completed with exit code `0`,
  totals/version are retained in deployment evidence, and an idempotent rerun
  has been rehearsed.
...
- UTC/Kyiv mistakes can silently move late-evening facts to a different report
  day; direct UTC-date extraction or UTC-midnight filters are prohibited.
- A recalculation-version deploy without the mandatory pre-traffic cache rebuild
  can make membership reads fail closed; release evidence must prevent traffic
  from starting on stale rows.
