...
| Issue membership | `_IssueMembershipForm`, issue view model, warning/command mappings | Complete | Complete | input, inactive type, automatic negative coverage, stale/warning and payment failures mapped | bilingual representative action plus tablet/phone issue workflow | T/P form/success captures; Reception Matrix |
