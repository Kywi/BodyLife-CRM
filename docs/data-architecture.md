...
| `issued_memberships` | `id`, `client_id`, `membership_type_id`, `issuance_mode`, `type_name_snapshot`, `duration_days_snapshot`, `visits_limit_snapshot`, `price_amount_snapshot`, `price_currency_snapshot`, `start_date`, `base_end_date`, `issued_at`, `issued_by_account_id`, `status`, `entry_origin`, `entry_batch_row_id`, `comment` | `client_id -> clients.id`, `membership_type_id -> membership_types.id` | `issuance_mode` is immutable `sale` or `opening_state`. ADR-021 / 21.2 stores `closed` beside active/sale-correction statuses as a non-correction lifecycle projection; 21.3 commands now perform atomic transitions. Snapshot fields are immutable. An active or closed ordinary sale has exactly one active membership_sale Payment linked through `payments.membership_id`; opening-state backfill is explicitly paymentless. |
...
| `membership_lifecycle_closures` | `id`, `client_id`, `source_membership_id`, optional `successor_membership_id` / `negative_closure_id`, `reason_code`, actor/session, correlation/idempotency, `entry_origin`, `entry_batch_id`, occurred/recorded times, optional explanation | Same-client Membership and negative-closure composite references | ADR-021 / 21.2 append-only explanation of `closed` in the sole greenfield `InitialBaseline`; one fact per source, distinct source/successor. Rollover uses a successor; one-off uses its originating aggregate. Paper linkage uses the existing `entry_batch_row_entities` aggregate row. |
...
- ADR-021 / 21.2 persistence: PostgreSQL partial unique index permits at most one `issued_memberships.status = active` row per Client. A closure fact has `UNIQUE(source_membership_id)`, rejects identical source/successor and uses same-client composite FKs for source, optional successor and originating negative closure. Deferred commit validation aligns each `closed` status with exactly one closure fact and active/canceled/corrected with none. UPDATE, DELETE and TRUNCATE cannot erase closure history. These facts and indexes belong only in the sole greenfield `InitialBaseline`; no legacy repair or production migration chain is added while deployment remains greenfield.
- Closure reason codes are `zero_balance_rollover`, `negative_balance_rollover` and `one_off_zero_balance`. Rollover requires a successor and no negative-closure reference; one-off requires the opposite. A filtered unique negative-closure reference prevents one aggregate from newly closing two Memberships. Deferred provenance validation requires a one-off method and historical item participation by the source Membership; later canceled/replaced coverage remains valid evidence. Balance policy stays in Memberships commands/calculation, with atomic Issue/full-one-off transitions and correction dependencies implemented in 21.3.
