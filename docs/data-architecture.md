...
| `freezes` | `id`, `client_id`, `membership_id`, `start_date`, `end_date`, `reason`, `occurred_at`, `recorded_at`, `recorded_by_account_id`, `session_id`, `entry_origin`, `entry_batch_id`, `status` | `membership_id -> issued_memberships.id` | Inclusive range. Active freezes contribute extension dates. |
| `freeze_cancellations` | `id`, `freeze_id`, `reason`, `recorded_at`, `recorded_by_account_id`, `session_id` | `freeze_id -> freezes.id` | Source fact. Recalculation removes freeze dates from active extension sources. |
