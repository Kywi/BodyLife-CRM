...
Дата: 2026-07-22
Оновлено: 2026-08-10 — current-Home composition accepted for the editable lab
and applied across its route inventory; production Wave acceptance is unchanged.
...
### Editable-lab composition decision (2026-08-10)

The approved evolved `docs/ui-prototype/index.html` is the canonical composition
baseline for the editable review lab and remaining template migration. It keeps
the current Home desktop rail and sticky header with global Search/direct Create
Client, Activity focal and Attention/Today contextual. Locked image/manifest
assets remain immutable historical/audit references; they do not force a return
to the earlier sidebar/topbar/Quick Search composition. This decision does not
approve a production Razor wave, alter the Wave ledger, or replace explicit
production side-by-side acceptance.

- Phase 0 copied and hash-verified the immutable `bodylife-light-v1` package
  and recorded delivered capture metadata. It is historical provenance; its
  earlier composition/tokens no longer override the evolved current-Home lab.
...
- **Production stop/go is active:** do not begin production Waves 2–6 until the user/product owner
...
Поточний production Razor UI є функціонально валідованим baseline, але не є
автоматично approved visual-fidelity result. Для editable lab візуальним
authority є evolved current Home: світлий operational canvas, desktop rail,
sticky header із глобальним Search і direct Create Client, Activity як focal
surface, Attention/Today як context, чесне account menu та semantic check-in
signals. Решта lab pages наслідує цю композицію через shared tokens/patterns,
а не повертається до раннього locked screenshot layout.

Ієрархія джерел:

1. Accepted ADRs, contracts і бізнес-workflows керують поведінкою.
2. Approved evolved `docs/ui-prototype/index.html` і
   `.interface-design/system.md` керують editable-lab композицією та tokens.
3. Цей план і `ui-visual-fidelity-coverage-matrix.md` керують migration
   охопленням та прийманням.
4. Locked references/manifest hashes зберігають historical delivery
   provenance; production Razor candidate потребує окремого route/state
   acceptance.
...
Current-Home composition is binding for editable templates: compact desktop
rail with Home/Clients/Report/History; sticky header with the transparent
BodyLife lockup, global Search, direct `Create Client` and honest account menu;
Reception Home has `Activity now` as its broad focal surface and
`Attention`/`Today` as the contextual rail. Profile orders identity, critical
warnings, Membership state, actions, Context and history. Create Client is a
direct dedicated anchor; Cancel Visit is an expanded danger/correction anchor
that keeps the original fact visible. Phone reorders every area into one
operational column without clipping warnings, status or correction actions.

### Primary navigation mapping

The compact four-item current-Home navigation does not remove any route:

| Navigation item | Destination and active state |
| --- | --- |
...
### Current-Home light palette and shape

- Use the current-Home neutral light canvas and soft neutral cards, not a white
  square-card utility page.
- High-frequency primary CTA is near-black. Blue remains
...
- Green means active/success, amber means ending/low/zero/attention, red-orange
  ...
  text.
- Use the transparent BodyLife logo at the calm current-Home proportions in
  the sticky header and shared shell; do not recreate it as text or place it on
  a dark photographic background.
- Exact editable-lab background/surface/text/border/semantic colors,
  typography, radii, spacing and elevations live in
  `.interface-design/system.md` and the shared prototype tokens. The locked
  HTML/PNG manifest remains audit input, not the current composition target.
...
## Production waves and executable gates

Every production wave is **not started → in progress → approved**. One writer
only; independent read-only review follows each writer. Do not begin a broad
production CSS pass before the relevant real Razor anchor composition is
approved. Completion of the editable-lab migration does not advance this
ledger.

| Wave | Status | Work and likely files | Stop/go |
| --- | --- | --- | --- |
| 0 | completed / locked | Lock the historical repository reference package and capture metadata; record the delivered tokens; update workflow/interaction docs with Activity and Attention contracts, navigation mapping, deterministic fixture and visual-test manifest. No production UI composition changes. | All 12 delivered artifact hashes match; capture metadata, contracts, typed failures and ownership were independently reviewed. The evolved current-Home decision supersedes the package as editable-lab composition. |
...
composition. Compare the real Razor candidate with current-Home at tablet
`1024x768` and phone `390x844`; historical reference-native captures are
...
- **P1**: material shell/navigation/logo/composition/palette mismatch, missing
  required canonical data block, unreadable semantic hierarchy, keyboard/focus failure
