...
Джерело: accepted ADR package у `docs/adr/`, ADR-001..ADR-013, accepted 2026-07-07.
...
- `Visits`: owns visit source records, cancellations and visit commands. It must trigger Memberships recalculation and business audit after successful state changes. (ADR-005, ADR-006, ADR-007)
...
- `Freezes`: owns freeze source records and cancellation/correction workflow. It does not compute final membership extension alone; Memberships computes effective state and overlap rules. (ADR-004, ADR-005, ADR-012)
...
- ADR traceability gate: each module boundary, command rule, report rule, audit rule and out-of-scope rejection must cite ADR-001..ADR-013 or stay out of the baseline.
